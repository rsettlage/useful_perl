...
#
#	use: perl /groups/DAC/useful_perl/filterReads_byBLAST_SE_separate_reads_file.pl <reads to keep blast table extension> <reads1.fastq>
...
my $blastfile = $ARGV[1].$ARGV[0];   #####<----------------------change this

open(fastq_FILE1, $fastqfile)|| die "Couldn't open fastq file: $fastqfile\n";
open(blast_FILE1, $blastfile)|| die "Couldn't open blast file: $blastfile\n";

my $base=$fastqfile;
##$base=~ s/fastq$//; ##my ($base, $fastq=split(/\./, $fastqfile);  ##this is dangerous, should just do a s/fastq//
my $out_hit=$base . ".end_hit.fastq";  #####<----------------------change this
my $out_nohit=$base . ".end_nohit.fastq";  #####<----------------------change this
open(OUT_HIT, ">$out_hit") || die "Couldn't open out hit file: $out_hit\n";
open(OUT_NOHIT, ">$out_nohit") || die "Couldn't open out nohit file: $out_nohit\n";
