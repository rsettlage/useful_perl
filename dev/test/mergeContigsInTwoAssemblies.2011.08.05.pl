...
3. blast for itself. (it needs -F F -e 1e-10 ). Let's assume output as 'merged.bln.raw'

4. convert the blast raw result to a blast table format with 'blast_table.pl -len'
(./blast_table.pl -len  merged.bln.raw  merged.bln.table). The '-len' option will add length information of queries.

5. ./mergeContigsInTwoAssemblies.pl -b merged.bln.table  -s merged.fa  -o merged.new.fa
...
sub help{
	...
	print STDERR " Usage: $0  -b <blast table file (from 'blastall -m 8 option'>  -s <contig FASTA file>   -o <output FASTA file> \n\n";
	...
	print STDERR "        [ -454 <header of contigs from 454 reads (default : contig) (ex. velvet contigs : 'NODE_'. )> \n";
	print STDERR "            When two contigs from different assemblies overlap, \n";
	print STDERR "             it will choose non-454 contig because of homopolymer errors in 454 contigs]\n";
	...
	print STDERR "  --------------------------------------------------\n\n";
	print STDERR "  ex) $0 -b mergedContigs.NoMT.NoInc.bln.table.gz -s mergedContigs.NoMT.NoInc.fa -S Good.Step6Len12/assembly/454Scaffolds.txt -o mergedContigs.NoMT.NoInc.new.fa\n";
	exit($return);
}
...
while(<$in>){
	...
	@arr = split /\t/;
#	if($#arr < 13){
#		print STDERR "The blast file does not have length information of query(13th column) and database(14th column).\n";
#		print STDERR " It can be added by  the 'blast_table.pl -len' command.\n";
#		close($in);
#		exit;
#	}
	...
	for($i = 0; $i < $nodeNum - 1; $i++){
		...
		if($strandList[$i] == $F) { 
			...
			if($start <= $match->{qStart}){
				...
				if( ($nodeList[$i] !~ /^$head454/ && $nodeList[$i+1] =~ /^$head454/) || ### avoid using 454 contigs.
							(
								($nodeList[$i] !~ /^$head454/ && $nodeList[$i+1] !~ /^$head454/ || $nodeList[$i] =~ /^$head454/ && $nodeList[$i+1] =~ /^$head454/) && 
								$match->{qEnd}-$match->{qStart} <= abs($match->{rEnd}-$match->{rStart})  #### choose shorter contig.
							)
					){ 
